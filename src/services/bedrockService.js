...
class BedrockService {
    ...
    async extractEventDetailsForPipeline(userMessage) {
        try {
            const extractionPrompt = this.buildPipelineExtractionPrompt(userMessage);
            ...
            const result = this.parseStructuredResponse(response, 'event_extraction');
...
Have a great day!`;
    ...
    async processOutfitPlanningRequest(userMessage) {
        try {
            // Step 1: Extract event parameters from unstructured input
            const eventData = await this.extractEventParameters(userMessage);

            if (!eventData.success) {
                return eventData; // Return error if extraction failed
            }

            // Step 2: Get weather data if location is provided
            let weatherData = null;
            if (eventData.data.location && eventData.data.startDate) {
                try {
            ...
            // Step 3: Return structured event context
            ...
            return {
                ...
                data: {
                    ...
                    needsClarification: eventData.data.needsClarification || [],
